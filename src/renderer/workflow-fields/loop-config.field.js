...
function renderFilterField(source, filter) {
  ...
  <label class="wf-step-edit-label">Pattern glob</label>
  ...
  if (source === 'custom') {
    return `<div class="wf-step-edit-field">
  <label class="wf-step-edit-label">Items</label>
...
function renderSchema(itemSchema) {
  ...
  <div class="wf-loop-schema-title">Structure de $item</div>
...
function renderUsageHint(itemSchema) {
  const schemaKeys = (itemSchema && itemSchema.length > 0)
    ? itemSchema.map(key => `<code>$item.${esc(key)}</code> <span>Champ "${esc(key)}"</span>`).join('')
    : '';
  return `<div class="wf-loop-usage-hint">
  <div class="wf-loop-usage-title">Variables dans Each</div>
  <div class="wf-loop-usage-items">
    <code>$item</code> <span>Élément courant</span>
    ${schemaKeys}
...
<div class="wf-step-edit-field">
  <label class="wf-step-edit-label">Source d'itération</label>
  ...
  <select class="wf-step-edit-input wf-node-prop wf-loop-source-select" data-key="source">
    <option value="auto"${(!source || source === 'auto' || source === 'previous_output') ? ' selected' : ''}>Automatique (depuis node connecté)</option>
    <option value="projects"${source === 'projects' ? ' selected' : ''}>Tous les projets enregistrés</option>
    <option value="files"${source === 'files' ? ' selected' : ''}>Fichiers (pattern glob)</option>
    <option value="custom"${source === 'custom' ? ' selected' : ''}>Liste personnalisée</option>
  </select>
</div>
...
<div class="wf-loop-options">
  <div class="wf-loop-opt">
    ...
    <div class="wf-loop-mode-tabs">
      <button class="wf-loop-mode-tab${mode === 'sequential' ? ' active' : ''}" data-mode="sequential" title="Un par un dans l'ordre">Séq.</button>
      <button class="wf-loop-mode-tab${mode === 'parallel' ? ' active' : ''}" data-mode="parallel" title="Tous en parallèle">Par.</button>
    </div>
  </div>
  <div class="wf-loop-opt">
  ...
  bind(container, field, node, onChange) {
    ...
    container.querySelectorAll('.wf-loop-mode-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        container.querySelectorAll('.wf-loop-mode-tab').forEach(t => t.classList.remove('active'));
