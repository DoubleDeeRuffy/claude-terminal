...
expected: Open a fresh terminal (not resuming any session). It should work normally — no errors, no unexpected claudeSessionId attached. Starting a new chat session should behave as before.
result: pass
...
- truth: "Session ID survives multiple restart cycles - after /clear new session ID is persisted"
  ...
  severity: major
  test: 1
  root_cause: "After /clear creates a new SDK session, the new session_id is captured in local variable sdkSessionId (ChatView.js:3047) but never propagated back to termData.claudeSessionId via updateTerminal(). TerminalSessionService reads the stale value and persists the old ID to disk."
  artifacts:
    - path: "src/renderer/ui/components/ChatView.js"
      issue: "Line 3047: sdkSessionId updated locally but never written to termData"
    - path: "src/renderer/ui/components/TerminalManager.js"
      issue: "createChatTerminal() onSessionStart callback doesn't set claudeSessionId on termData"
  missing:
    - "Update termData.claudeSessionId when msg.session_id changes in ChatView streaming loop"
    - "Set claudeSessionId on termData in createChatTerminal onSessionStart callback"
  debug_session: ".planning/debug/session-resume-after-clear.md"
