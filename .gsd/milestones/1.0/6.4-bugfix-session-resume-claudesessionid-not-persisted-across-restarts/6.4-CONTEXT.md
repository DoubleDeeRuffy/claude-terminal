...
Fix the bug where `claudeSessionId` is not reliably persisted in `terminal-sessions.json`, causing Claude sessions to start fresh instead of resuming after app restart. Three diagnosed root causes (A, B, C) are all in scope. Debug investigation in `.gsd/debug/session-not-resumed-on-restart.md` has full evidence.
...
- Debug investigation is complete — `.gsd/debug/session-not-resumed-on-restart.md` contains full root cause analysis with line numbers and evidence
