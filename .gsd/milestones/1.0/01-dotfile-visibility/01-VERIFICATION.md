...
gaps: []
human_verification:
  - test: "Expand a project that contains dotfolders like .gsd or .github in the file explorer"
...
|---|-------|--------|---------|
| 1 | Dotfolders (.gsd, .claude, .github, .husky, etc.) appear in the file explorer tree when expanding a project root | VERIFIED | `readDirectoryAsync` loop at line 231–250 has no `name.startsWith('.')` guard; only `IGNORE_PATTERNS.has(name)` (line 232). Grep confirms 0 `startsWith('.')` filter occurrences in the entire function. |
...
|-------------|------------|-------------|--------|---------|
| FILE-01 | 01-01-PLAN.md | User can see dotfiles and dotfolders (.gsd, .git, etc.) in file explorer tree | SATISFIED | `readDirectoryAsync` no longer filters on `name.startsWith('.')`. Verified by grep: 0 matches. REQUIREMENTS.md marks FILE-01 as `[x]` (complete). |
...
**Test:** Open the app, select a project that contains `.gsd` or `.github`, expand the project root in the file explorer.
**Expected:** `.gsd`, `.github`, and other dotfolders appear in the tree alongside regular folders.
