...
**One-liner:** Removed blanket `name.startsWith('.')` dotfile filter from both `readDirectoryAsync` and `collectAllFiles` in FileExplorer.js so dotfolders like `.gsd`, `.claude`, and `.github` appear in the tree view and Ctrl+P search while IGNORE_PATTERNS entries remain hidden.
