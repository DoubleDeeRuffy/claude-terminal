...
Remove the second filter line (the dotfile check) entirely. The `IGNORE_PATTERNS` set already handles `.git`, `.DS_Store`, etc. explicitly. The generic `name.startsWith('.')` check is what hides `.gsd`, `.claude`, `.github`, and other legitimate dotfiles.
...
- `.gsd/codebase/ARCHITECTURE.md` — layered IPC analysis
