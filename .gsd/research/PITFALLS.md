...
Removing the filter from only `readDirectoryAsync()` makes dotfiles visible in the tree but invisible to search (Ctrl+P file search). Users will be confused when they can see `.gsd/` in the tree but cannot find files inside it through search.
...
- `.gsd` directory is visible but searching for `PITFALLS.md` returns no result
...
- [ ] **Dotfile visibility — search:** After removing the filter, open file search (Ctrl+P) and search for a file inside `.gsd/` — confirm it appears in results.
- [ ] **Dotfile visibility — tree:** Confirm `.git` does NOT appear (still blocked by `IGNORE_PATTERNS`), while `.gsd` DOES appear.
...
| Dotfile filter in two code paths | Implementation — grep before editing | After change: `.gsd/PITFALLS.md` must appear in both file tree AND Ctrl+P search |
